...
class Alumnos:
        ...
        try:
            ...
            if data['token'] == "1234":  # valida el token que se recibe por url
                if data['action'] == 'get':  # evalua la acción a realizar
                    ...
                    return json.dumps(result)  # Parsea el diccionario result a formato json
                elif data['action'] == 'put': # evalua la acción a realizar
                    matricula=data['matricula']
                    nombre=data['nombre']
                    primero=data['apellido_uno']
                    segundo=data['apellido_dos']
                    carrera=data['carrera']
                    result = self.actionPut(self.app_version, self.file, matricula,nombre,primero,segundo,carrera)
    ...
    @staticmethod
    def actionPut(app_version, file, matricula, nombre, primero, segundo, carrera):
        ingreso=[]
        ingreso.append(matricula)
        ingreso.append(nombre)
        ingreso.append(primero)
        ingreso.append(segundo)
        ingreso.append(carrera)
